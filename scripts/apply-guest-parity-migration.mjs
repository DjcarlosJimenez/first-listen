...
const migrationVersion = "20260610018000";
const migrationName = "guest_experience_parity";
const migrationPath =
  "supabase/migrations/20260610018000_guest_experience_parity.sql";
...
if (!apply) {
  await query(`
    begin;
    ${migrationSql}
    select public.guest_experience_parity_health_report();
    rollback;
  `);
  console.log(
    JSON.stringify({
...
const verification = await query(`
  select
    version,
    name,
    public.guest_experience_parity_health_report() as health
  from supabase_migrations.schema_migrations
  where version = '${migrationVersion}';
`);

console.log(JSON.stringify({ status: "applied", migration: verification[0] }));
