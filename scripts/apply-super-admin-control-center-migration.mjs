...
const migrationVersion = "20260611000000";
const migrationName = "super_admin_control_center";
const migrationPath =
  "supabase/migrations/20260611000000_super_admin_control_center.sql";
...
await query(`
  ...
  values (
    '${migrationVersion}',
    '${migrationName}',
    array['Applied through the audited First Listen control-center runner.']
  );
  commit;
`);
...
const verification = await query(`
  select
    version,
    name,
    public.super_admin_control_center_health_report() as health
  from supabase_migrations.schema_migrations
  where version = '${migrationVersion}';
`);
